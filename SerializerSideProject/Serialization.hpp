        ...
        void GetContainerValue(Container<Type, Containee...>& valueContainer, const std::string& Key)
        {
            ...
            if constexpr (TYPETRAITS::is_pair<std::decay_t<decltype(*valueContainer.begin())>>::value)
            {
                ...
                if (data.IsArray())
                {
                    ...
                    {
                        const Type key = GetValue<Type>(m->name.GetString());
                        auto newValue = 1;

                        if constexpr (TYPETRAITS::areSame<uint64_t, Containee...>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., int64_t>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., double>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., float>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., bool>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., int>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., unsigned>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }

                        if constexpr (TYPETRAITS::areSame<Containee..., std::string>::value || TYPETRAITS::areSame<Containee..., const char*>::value)
                        {
                            Containee value = GetValue<Containee>(m->value);
                            newValue = value;
                        }
